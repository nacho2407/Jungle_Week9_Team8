...
namespace
{
constexpr int32 EditorHelperBVHMaxTraversalStack = 512;
...
void FEditorHelperBVH::MarkDirty()
{
    bDirty = true;
}

void FEditorHelperBVH::BuildNow(const TArray<AActor*>& Actors)
{
    Leaves.clear();
    BVH.Reset();
    ...
    for (AActor* Actor : Actors)
    {
        ...
        {
            if (!Primitive || !Primitive->ShouldRenderInCurrentWorld() || !Primitive->IsEditorHelper() || !IsPickableEditorHelper(Primitive))
            {
                continue;
            }
...
void FEditorHelperBVH::EnsureBuilt(const TArray<AActor*>& Actors)
{
    if (!bDirty)
    {
        return;
...
bool FEditorHelperBVH::Raycast(const FRay& Ray, FHitResult& OutHitResult, AActor*& OutActor) const
{
    ...
    FTraversalEntry NodeStack[EditorHelperBVHMaxTraversalStack];
    int32 StackSize = 0;
    ...
    while (StackSize > 0)
    {
        ...
        {
            if (StackSize < EditorHelperBVHMaxTraversalStack)
            {
        ...
        for (int32 I = 0; I < ChildEntryCount && StackSize < EditorHelperBVHMaxTraversalStack; ++I)
        {
