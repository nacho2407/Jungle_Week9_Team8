...
void FRenderer::Create(HWND hWindow)
{
    ...
    TileCulling.Initialize(&Device);
    TileCulling.ResizeTiles(Device.GetViewport().Width, Device.GetViewport().Height);
    ...
    if (!Proxy.SectionDraws.empty())
    {
        for (const FMeshSectionDraw& Section : Proxy.SectionDraws)
        {
            ...
            if (Pass == ERenderPass::Z_Prepass)
            {
                ...
                Cmd.DepthStencil = EDepthStencilState::Default; // Depth Write On
                Cmd.Rasterizer = ERasterizerState::SolidBackCull;
                Cmd.DiffuseSRV = nullptr; // 텍스처 샘플링 불필요
            }
            else
            {
...
{
    if (!ReceiverProxy.MeshBuffer || !ReceiverProxy.MeshBuffer->IsValid())
        return;
    if (!DecalProxy.Shader || !DecalProxy.DiffuseSRV)
        return;
    ...
    auto AddDraw = [&](uint32 FirstIndex, uint32 IndexCount)
    {
        if (IndexCount == 0)
            return;
        if (DecalPass == ERenderPass::Decal && CollectViewMode == EViewMode::Wireframe)
            return; // 데칼 패스는 와이어프레임 모드에서 스킵
...
void FRenderer::Render(const FFrameContext& Frame)
{
    ...
    BuildDefaultPassEvents(PrePassEvents, Context, Frame, Cache, ActiveViewPipeline, ActiveViewSurfaces, TileCulling, Resources);
